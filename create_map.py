...
# Cargar los datos del GeoJSON generado por process_data.py
gdf = gpd.read_file('arsat2_coverage.geojson')

# Crear un mapa centrado en Sudamérica
m = folium.Map(location=[-30, -60], zoom_start=4)

# Función para convertir color hexadecimal a RGB
def hex_to_rgb(hex_color):
    hex_color = str(hex_color).lstrip('#')
    # Asegurarse de que el valor de color tenga 6 caracteres
    if len(hex_color) == 3:
...
for _, row in gdf.iterrows():
    ...
    color_hex = f"{int(row['color']):06x}"  # Asegurar que tenga 6 dígitos
    color_rgb = hex_to_rgb(color_hex)
    
    # Crear el polígono con estilo
    folium.GeoJson(
        row['geometry'],
